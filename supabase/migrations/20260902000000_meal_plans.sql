-- Meal planning.
--
-- Named meal plans, dated entries in per-plan meal slots, and the
...
create table meal_plan_entries (
  ...
  slot      text not null constraint meal_plan_entries_slot_not_blank check (slot <> ''),
  position  smallint not null default 0,
  created_at timestamptz not null default now(),
  unique (plan_id, date, slot, recipe_id)
);

create index meal_plan_entries_plan_date_idx
...
create table meal_plan_invites (
  id          uuid primary key default gen_random_uuid(),
  plan_id     uuid not null references meal_plans(id) on delete cascade,
...
create index meal_plan_invites_plan_id_idx
